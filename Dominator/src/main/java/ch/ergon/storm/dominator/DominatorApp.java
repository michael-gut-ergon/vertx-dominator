...
public class DominatorApp {
	public static final Logger LOGGER = LoggerFactory.getLogger(DominatorApp.class);
	public static Configuration CONF;
	public static DbUtil DB;

	public static void main(String[] args) {
		LOGGER.info("DominatorApp start...");
		try {
			...
			Vertx.clusteredVertx(options, res -> {
				if (res.succeeded()) {
					Vertx vertx = res.result();
				  	vertx.deployVerticle(new DominatorVerticle(), stringAsyncResult -> {
				  		System.out.println("DominatorVerticle deployment complete");
				  	});
					...
					// failed!
					LOGGER.error("Dominator cluster manager start failed");
                }
            });
        } catch (IOException e) {
            LOGGER.error("Dominator start exception ");
            e.printStackTrace();
        }
    }
